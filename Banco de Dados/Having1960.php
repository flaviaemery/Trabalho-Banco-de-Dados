...
$sql = "SELECT name_ar AS Artista,  title AS Album, year AS Ano FROM Artist Ar INNER JOIN Edges E ON (Ar.id_ar = E.to_ed) INNER JOIN Albums Al ON (Al.id_al = E.from_ed) HAVING year <= '1960' ORDER BY year desc;";
$resultado = mysqli_query($strcon,$sql) or die ("Erro ao tentar cadastrar registro");

//Obtendo os dados por meio de um loop while
while ($registro = mysqli_fetch_array($resultado))
{
  $col1 = $registro['Artista'];
  $col2 = $registro['Album'];
  $col3 = $registro['Ano'];
  echo "<tr>";
  echo "<td>".$col1."</td>";
